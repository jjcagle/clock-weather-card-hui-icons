...
const iconMap: Record<string, string> = {
  'clear-night': '/local/weather/moon.stars.fill.svg',
  clear: '/local/weather/sun.max.fill.svg',
  sunny: '/local/weather/sun.max.fill.svg',
  partlycloudy: '/local/weather/cloud.sun.fill.svg',
  cloudy: '/local/weather/cloud.fill.svg',
  overcast: '/local/weather/cloud.fill.svg',
  fog: '/local/weather/cloud.fog.fill.svg',
  hazy: '/local/weather/cloud.fog.fill.svg',
  mist: '/local/weather/cloud.fog.fill.svg',
  rainy: '/local/weather/cloud.rain.fill.svg',
  pouring: '/local/weather/cloud.heavyrain.fill.svg',
  snowy: '/local/weather/cloud.snow.fill.svg',
  'snowy-rainy': '/local/weather/cloud.sleet.fill.svg',
  hail: '/local/weather/cloud.hail.fill.svg',
  sleet: '/local/weather/cloud.sleet.fill.svg',
  windy: '/local/weather/wind.svg',
  'windy-variant': '/local/weather/wind.svg',
  lightning: '/local/weather/cloud.bolt.fill.svg',
  'lightning-rainy': '/local/weather/cloud.bolt.rain.fill.svg',
  exceptional: '/local/weather/exclamationmark.triangle.fill.svg'
}
...
export class ClockWeatherCardHUIIcons extends LitElement {
  ...
  private toIcon (weatherState: string, forceDay: boolean): TemplateResult {
    const isNight = !forceDay && this.getSun()?.state === 'below_horizon'
    let state = weatherState

    if (isNight && (state === 'clear' || state === 'sunny')) {
      state = 'clear-night'
    }

    const icon = iconMap[state] ?? iconMap.clear

    return html`<img class="weather-icon" alt="${state}" src="${icon}" />`
  }
